...
def encode(y):
	...
	return np.array(classes_expanded == np.column_stack((y,)), dtype = float)
...
def descent(X, y, w, stochastic = False, batch_size = 0):
	...
	#alpha = minimize(lambda alpha: evalLoss(X, y, w - alpha*grad_w), 0, method = 'BFGS', jac = lambda alpha: np.array([np.trace(-1*evalGrad(X, y, w - alpha*grad_w).T.dot(grad_w))])).x[0]
	alpha = 2
